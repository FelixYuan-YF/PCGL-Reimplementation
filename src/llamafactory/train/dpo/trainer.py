...
from typing import TYPE_CHECKING, Literal, Optional, Union, Dict, List
...
class CustomDPOTrainer(DPOTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.humor_gamma = finetuning_args.humor_gamma
    ...
    def concatenated_forward(
    ...
    ) -> tuple["torch.Tensor", "torch.Tensor", Optional["torch.Tensor"], "torch.Tensor", "torch.Tensor", Optional["torch.Tensor"], "torch.Tensor"]:
        ...
        if self.loss_type == "humor":
            ...
            chosen_logps, rejected_logps, gold_logps = all_logps.split(batch_size, dim=0)
            chosen_logits, rejected_logits, gold_logits = all_logits.split(batch_size, dim=0)
        ...
        if self.loss_type in ["ipo", "orpo", "simpo"]:
            return chosen_logps, rejected_logps, None, chosen_logits, rejected_logits, None, chosen_logps / chosen_length
        else:
            return chosen_logps, rejected_logps, gold_logps if self.loss_type == "humor" else None, chosen_logits, rejected_logits, gold_logits  if self.loss_type == "humor" else None, chosen_logps / chosen_length
    ...
    def compute_reference_log_probs(
        ...
        with torch.no_grad(), ref_context:
            reference_chosen_logps, reference_rejected_logps, reference_gold_logps, *_ = self.concatenated_forward(ref_model, batch)

        return reference_chosen_logps, reference_rejected_logps, reference_gold_logps
    ...
    def concatenated_inputs(
        ...
        """
        concatenated_batch = {}
        print(batch["gold_input_ids"].size())
