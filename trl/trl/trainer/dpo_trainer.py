...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        beta: float = 0.1,
        gamma: float = 0.1,
        loss_type: Literal["sigmoid", "hinge", "custom"] = "sigmoid",
    ...
    def dpo_loss(
        ...
        policy_gold_logps: torch.FloatTensor = None,
        reference_gold_logps: torch.FloatTensor = None,
        ...
        if self.loss_type == "sigmoid":
            ...
            losses = -F.logsigmoid(self.beta * logits) + self.gamma * (-F.logsigmoid(self.beta * gold_logits))
        else:
            raise ValueError(f"Unknown loss type: {self.loss_type}. Should be one of ['sigmoid', 'hinge', 'custom']")
        ...
        if self.loss_type == "custom":
            ...
            return losses, chosen_rewards, rejected_rewards, gold_rewards
        else:
    ...
    def get_batch_metrics(
        ...
        if self.loss_type == "custom":
            losses, chosen_rewards, rejected_rewards, gold_rewards = self.dpo_loss(
                policy_chosen_logps=policy_chosen_logps,
                policy_rejected_logps=policy_rejected_logps,
                policy_gold_logps=policy_gold_logps,
                reference_chosen_logps=reference_chosen_logps,
                reference_rejected_logps=reference_rejected_logps,
                reference_gold_logps=reference_gold_logps,
            )
        else:
        ...
        if self.loss_type == "custom":
            metrics[f"{prefix}rewards/gold"] = gold_rewards.cpu().mean()
            metrics[f"{prefix}rewards/gold_vs_chosen"] = (gold_rewards - chosen_rewards).cpu().mean()
            metrics[f"{prefix}rewards/gold_vs_rejected"] = (gold_rewards - rejected_rewards).cpu().mean()
        metrics[f"{prefix}rewards/accuracies"] = reward_accuracies.cpu().mean()
        metrics[f"{prefix}rewards/margins"] = (chosen_rewards - rejected_rewards).cpu().mean()
        metrics[f"{prefix}logps/rejected"] = policy_rejected_logps.detach().cpu().mean()
        metrics[f"{prefix}logps/chosen"] = policy_chosen_logps.detach().cpu().mean()
        if self.loss_type == "custom":
            metrics[f"{prefix}logps/gold"] = policy_gold_logps.detach().cpu().mean()
        metrics[f"{prefix}logits/rejected"] = policy_rejected_logits.detach().cpu().mean()
        metrics[f"{prefix}logits/chosen"] = policy_chosen_logits.detach().cpu().mean()
        if self.loss_type == "custom":
            metrics[f"{prefix}logits/gold"] = policy_gold_logits.detach().cpu().mean()
